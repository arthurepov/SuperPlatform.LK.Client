...
export const OrganizationPage: React.FC = () => {
  ...
  const { organizations, schedules, disciplines, loading } =
    useStore($OCardStore);
  ...
  const currentOrganization = useMemo(
    () =>
      organizations.data?.find(
        ({ id: organizationId }) => organizationId === Number(id)
      ),
    [organizations, schedules, disciplines, loading]
  );
      ...
      schedules.data?.reduce((acc, curr) => {
        if (
          ...
          if (disciplineIndex >= 0) {
            acc[disciplineIndex] = {
              ...acc[disciplineIndex],
              ageMin: acc[disciplineIndex]?.ageMin
                ? acc[disciplineIndex]?.ageMin
                : curr?.ageMin,
              ageMax: acc[disciplineIndex]?.ageMax
                ? acc[disciplineIndex]?.ageMax
                : curr?.ageMax,
