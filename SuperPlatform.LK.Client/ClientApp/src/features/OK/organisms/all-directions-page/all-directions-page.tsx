...
import { HobbySlider, OrganizationSlider, SliderList, WrapList } from '../../molecules';
import { NoData } from '../../atoms';

import s from './all-directions-page.module.scss';

export const AllDirectionsPage = () => {
    ...
    const filteredHobbies = query.length
        ? hobbies?.data?.filter(({ name }) => name.toLowerCase().includes(query.toLocaleLowerCase()))
        : [];

    const filteredDisciplines = query.length
        ? disciplines?.data?.filter(({ name }) => name.toLowerCase().includes(query.toLocaleLowerCase()))
        : [];

    const filteredOrganizations = query.length
        ? organizations?.data?.filter(({ name }) => name.toLowerCase().includes(query.toLocaleLowerCase()))
        : [];
