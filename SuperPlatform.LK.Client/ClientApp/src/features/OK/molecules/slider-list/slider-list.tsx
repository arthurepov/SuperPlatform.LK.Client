...
export const SliderList: React.FC<ISliderList> = ({ array, title: sliderTitle, onClick, buttonText, pushState }) => {
    const params = {
        ...
        <>
            {
                (sliderTitle || buttonText) && (
                    <div className={s.header}>
                        ...
                        {buttonText && <Button onClick={onClick} variant="link">{buttonText}</Button>}
                    </div>
                )
            }
            <Swiper {...params}>
                {
                    array.map(({ id, name, ageMin, ageMax, photo, organizationsCount = 0 }) => (
                        <Link
                            to={{
