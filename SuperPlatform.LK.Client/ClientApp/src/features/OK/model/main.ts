...
const $directionsStatus = status({ effect: getDisciplinesFx, defaultValue: 'pending' });
const $disciplinesStatus = status({ effect: getDisciplinesFx, defaultValue: 'pending' });
const $organizationsStatus = status({ effect: getOrganizationsFx, defaultValue: 'pending' });
const $schedulesStatus = status({ effect: getSchedulesFx, defaultValue: 'pending' });
    ...
    (
        ...
        disciplines: {
            ...
            data: disciplines.data
                ...
                ?.filter(({ id }) => {
                    const organizationIds = [...new Set(schedules?.data?.map((({ disciplineId }) => disciplineId)))];

                    return organizationIds.includes(id);
                })
                .map((discipline) => {
                    const disciplineSchedules = schedules.data
                            ?.filter(({ disciplineId }) => disciplineId === discipline.id) ?? [];
                    const disciplineOrganizations = disciplineSchedules
        ...
        organizations: {
            ...organizations,
            data: organizations.data?.filter(({ name, id }) => name !== null && id !== null) ?? null
        },
        schedules,
        hobbies: {
            ...hobbies,
            data: [...new Set(schedules?.data?.map(({
                name,
                organizationId,
                address,
                disciplineId,
                ...
                .map((hobby: string) => {
                    ...
                    const organization = organizations?.data?.find(({ id }) => id === data?.organizationId);
...
forward({
    ...
    to: [getDirectionsFx, getCitiesFx, getDisciplinesFx],
});
