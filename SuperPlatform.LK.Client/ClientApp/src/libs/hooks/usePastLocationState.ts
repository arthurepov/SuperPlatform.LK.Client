...
export const usePastLocationState = (defaultPastLocation?: IDefaultPastLocation): IUsePastLocationState => {
    const history = useHistory();
    const pastLocation = history.location.state as ILocationState;
    const title = pastLocation?.title ?? defaultPastLocation?.title ?? 'На главную';
